...
//
//   this code concerns the simulation of the first exit time appearing in Example 1
//   on page 11: dX=(2+sin(X))dt+dB
...
//
//  Settings:
//  - starting value of the diffusion: xstart (line 463)
//  - exit interval: I=[x_min,x_max] (line 462 and line 464)
//  - constant kappa (see Herrmann-Zucca 2020, ESAIM M2NA) -- line 465
//  - number of simulations (actions, line 443)
//  - box size (parameter N=MaxNb_slices, line 444)
//  - parameter epsilon (espilon-greedy algorithm, line 445)
//
//  Outcomes:
//  - Computational times: in file "comp-times.txt" (line 529)
//  - Choice of the arm: in file "choice-arms.txt" (line 535)
//  - Exit times: in file "exit-times.txt" (line 547)
//  - Exit location: in file "exit-location.txt" (line 541)
//
//////////////////////////////////////////////////////////////////////////////////
//
//
...
{
    return (pow((2+sin(x)),2)+cos(x))/2;
}
double betafunc(double x)
{
    return exp(2*x+1-cos(x));
}
double betaplus(double x_min, double x_max)
{
    return betafunc(x_max);
}
double gammaplus(double x_min, double x_max)
{
    return gammafunc(x_max);
}
double gammaminus(double x_min, double x_max)
{
    return fmin(gammafunc(x_min),0);
}
double gammazero(double x_min, double x_max)
{
    return gammaplus(x_min,x_max)-gammaminus(x_min,x_max);
}
//////////////////////////////////////////////////////////////////////////////////
//
...
//
//////////////////////////////////////////////////////////////////////////////////
void BrExitSy(double t0, double& Tvalue)
//  simulation of the Brownian exit time from the interval [-1,1] with the starting point 0,
//  this algorithm uses a splitting between small times (inverse gaussian method) and
//  large times (exponential distribution method)
...
//
//////////////////////////////////////////////////////////////////////////////////
void BoxExit(double x_start, double x_min, double x_max, double kappa, double& T_value, double& X_value)
{
    T_value=0;
    X_value=0;
    double X_interm2=0;
    ...
    while (test0)
    {
        ...
        while (test1)
        {
            double Erand=-log(uniform(generator))/gammazero(x_min,x_max);
            ...
            {
                X_interm+=X_interm2;
                if ((betaplus(x_min,x_max)*Urand<betafunc(X_interm)) && (log(Wrand)<(gammaminus(x_min,x_max)*(K_interm-T_interm))))
                {
                    test1=false;
                    test0=false;
                    X_value=X_interm;
            ...
            if ((K_interm<=Erand) && (T_interm>K_interm))
            {
                ...
                if (betaplus(x_min,x_max)*Urand<betafunc(X_interm2))
                {
                    test1=false;
                    test0=false;
                    X_value=X_interm2;
            ...
            if ((K_interm>Erand) && (T_interm>Erand))
            {
                ...
                X_interm2=(x_min+x_max)/2+(x_max-x_min)*X_interm2/2;
                if (gammazero(x_min,x_max)*Vrand>gammafunc(X_interm2)-gammaminus(x_min,x_max))
                {
                    X_interm=X_interm2;
                    T_value+=Erand;
...
//
//
void DiffExit(double x_start, double x_min, double x_max, double kappa, int Nb_slices, double& T_value, double& X_value, int& Counter)
{
    ...
    while (test)
    {
        BoxExit(X_interm, localmin(x_min,x_max,X_interm,Nb_slices),localmax(x_min,x_max,X_interm,Nb_slices),kappa,T_interm,X_interm2);
        T_value+=T_interm;
        X_interm=X_interm2;
        Counter+=1;
...
//////////////////////////////////////////////////////////////////////////////////
 //
    ...
    for (int j=0; j<actions;j++)
    {
        arms[j+1]=0;
        start=clock();
        DiffExit(x_start,x_min,x_max,kappa,arms[j]+2,Tval[j],Xval[j],Counter);
        finish = clock();
        ...
        mu2hat[arms[j]]=(sojourn[arms[j]]*mu2hat[arms[j]]+pow(times[j],2))/ ((double) sojourn[arms[j]] +1.);
        sojourn[arms[j]]+=1;
        //
